...
const db_util_Func = require('./DB_util_functions.js')
// const textProcessing = require('../Scrape_Clean_functions/Text-processing.js')
...
async function dateComp(celebHandle){

        let dateFromDB = await db_util_Func.getDateFromDB(celebHandle)
...
function detailDateCompare(dateFromDB,currentDate){
        ...
        let dateCurrent_monthdb = dateCurrent[1]//'mm'
...
async function getDateFromFile(raw_comments_file){
    let date = await extractLastPostDateFromFile(raw_comments_file)
    return date;
} 


// dateComp('@kylieJenner')
